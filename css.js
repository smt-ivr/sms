...
:root {
    --primary: #007aff; /* צבע כחול נייטיב */
    --primary-dark: #005bb5;
    --bg-main: #f4f5f7;
    --bg-chat: #ffffff;
    --text-main: #1c1c1e;
...
.login-box { background: #fff; width: 90%; max-width: 400px; padding: 40px 30px; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.08); text-align: center; }
.login-header h2 { font-size: 24px; font-weight: 700; margin-bottom: 5px; color: var(--primary); }
...
input[type="text"], input[type="password"] { width: 100%; padding: 14px; border: 1px solid var(--border); border-radius: 12px; font-size: 15px; text-align: center; background: #fafafa; transition: 0.2s; }
input:focus { outline: none; border-color: var(--primary); background: #fff; box-shadow: 0 0 0 3px rgba(0,122,255,0.1); }
.btn-primary, .btn-secondary { width: 100%; padding: 14px; border: none; border-radius: 12px; font-size: 16px; font-weight: 500; cursor: pointer; display: flex; justify-content: center; align-items: center; gap: 8px; transition: 0.2s; color: #fff; }
...
.divider { margin: 20px 0; color: var(--text-muted); font-size: 13px; position: relative; }
...
.error-msg { color: var(--danger); font-size: 14px; margin-top: 15px; min-height: 20px; font-weight: 500; }

/* --- מבנה אפליקציה ראשית --- */
#app-screen { display: flex; width: 100%; height: 100%; max-width: 1200px; margin: 0 auto; background: var(--bg-chat); box-shadow: 0 0 20px rgba(0,0,0,0.05); position: relative; }

/* סרגל צד (אנשי קשר) */
#sidebar { width: 350px; background: #fff; border-left: 1px solid var(--border); display: flex; flex-direction: column; z-index: 10; }
.sidebar-header { padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border); background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); }
.sidebar-header h2 { font-size: 20px; font-weight: 700; margin: 0; }
.sidebar-actions { display: flex; gap: 5px; align-items: center; }
#contacts-list { flex: 1; overflow-y: auto; }
.contact-item { padding: 15px 20px; border-bottom: 1px solid var(--border); cursor: pointer; transition: 0.2s; }
...
.contact-top { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 5px; }
.contact-name { font-weight: 600; font-size: 16px; }
.contact-time { font-size: 12px; color: var(--text-muted); }
.contact-last-msg { font-size: 14px; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: flex; align-items: center; gap: 5px; }

/* אזור צ'אט */
#chat-area { flex: 1; display: flex; flex-direction: column; background: var(--bg-chat); position: relative; }
.chat-header { padding: 10px 20px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--border); background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); min-height: 65px;}
.chat-title-info h2 { font-size: 18px; font-weight: 700; margin: 0; }
.chat-title-info .subtitle { font-size: 13px; color: var(--text-muted); }
#chat-messages { flex: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; background: var(--bg-main); }

/* הודעות בועה בסגנון נייטיב */
.message { max-width: 75%; padding: 10px 14px; border-radius: 18px; font-size: 15px; line-height: 1.4; position: relative; word-wrap: break-word; display: inline-block; clear: both; }
...
/* שורת כתיבת הודעה */
#chat-input-area { padding: 12px 20px; background: #fff; border-top: 1px solid var(--border); display: flex; gap: 10px; align-items: flex-end; }
...
@media (max-width: 768px) {
    ...
    /* אנימציית החלקה בין המסכים בנייד */
...
.modal-box h3 { margin-bottom: 10px; color: var(--primary); }
