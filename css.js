...
.sidebar-header, .chat-header {
    ...
    justify-content: space-between; height: 65px; min-height: 65px; flex-shrink: 0; 
    z-index: 2; box-shadow: 0 1px 2px rgba(0,0,0,0.05);
}

.sidebar-header h2 { font-size: 18px; color: var(--text-dark); margin: 0; }
...
/* --- Sidebar Contacts --- */
#contacts-list { flex: 1; overflow-y: auto; background: white; min-height: 0; }
...
.contact-top { display: flex; justify-content: space-between; margin-bottom: 4px; align-items: baseline; width: 100%;}
.contact-name { font-weight: 600; font-size: 16px; color: var(--text-dark); direction: ltr; text-align: right; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;}
...
.contact-last-msg { 
    ...
    overflow: hidden; text-overflow: ellipsis; direction: rtl; display: block;
    width: 100%;
}

.status-icon { font-size: 14px !important; vertical-align: middle; margin-left: 4px; }
...
/* --- Chat Area --- */
#chat-messages {
    flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; z-index: 1; min-height: 0;
}
