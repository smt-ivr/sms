...
async function apiCall(endpoint, method='GET', body=null) {
    ...
    try {
        ...
        if(!res.ok) {
            // טיפול בדרישת עדכון פרופיל לחשבונות ישנים
            if(data.require_profile_update) {
                throw { isProfileUpdate: true, msg: "נדרש עדכון פרטים" };
            }
...
async function loginUser() {
    ...
    try {
        ...
        if(e.isProfileUpdate) {
            // טיפול בחשבונות ישנים שחסר להם טלפון ואימייל
                ...
                <table>
                    ...
                    \${tickets.map(t => \`
                        <tr>
                            ...
                            <td>\${t.owner_name} <span style="font-size:11px;color:gray">(\${t.email})</span></td>
                            <td>\${t.subject}</td>
            ...
            <div class="card">
                ...
                <table>
                    ...
                    \${codes.map(c => {
                        ...
                        const canSend = c.permissions.includes('SEND');
...
// ... שאר הפונקציות הרגילות (UserSystemModal, UserModal וכו') זהות לגרסה הקודמת ...
